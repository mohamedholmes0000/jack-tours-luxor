  ...
  return (
    <div>
      ...
      <div className="mt-8 grid gap-6">
        <GalleryImageForm />
        {images.length ? images.map((image) => <div key={image.id} className="grid gap-4 lg:grid-cols-[180px_1fr]"><div className="relative min-h-40 overflow-hidden bg-[var(--color-gray-100)]"><Image src={image.url} alt={image.alt} fill sizes="180px" className="object-cover" /></div><GalleryImageForm id={image.id} initialValues={{ url: image.url, alt: image.alt, category: image.category ?? "Experiences", relatedTourSlug: image.relatedTourSlug ?? "", order: image.order }} /></div>) : <p className="border border-[var(--color-gray-100)] bg-white p-5 text-sm text-[var(--color-gray-600)]">No database gallery images found. The public page still uses static fallback content.</p>}
      </div>
    </div>
  );
}
